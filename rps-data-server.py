...
def show_last_game():
	ga = g.db.execute('select user_id_1, user_id_2, win_player_1, win_player_2, '
			...
			'paper_player_2, scissors_player_2 from game where user_id_1 != 0 and '
			'user_id_2 != 0 order by game_id desc limit 0,1').fetchone()
	if not ga:
		return 'Last game not found', 404
	player1 = g.db.execute('select name from user where user_id = ?',
			(ga[0],)).fetchone()
	player2 = g.db.execute('select name from user where user_id = ?',
			(ga[1],)).fetchone()
	if not (player1 and player2):
		return 'Player not found', 500
	result = {'name_1' : player1[0], 'name_2' : player2[0], 'won_1' : ga[2],
...
@app.route('/highscore')
def show_highscore():
	user = g.db.execute('select user_id, name, highscore from user where '
	...
	for u in user:
		...
		result.append({'score': u[2], 'name' : u[1], 'won' : won})
